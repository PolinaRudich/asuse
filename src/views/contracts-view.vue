  ...
  <div id="contracts-view">
    <div class="contracts-content">
      <div style="display: flex">
        <button
            class="content-h active-btn"
            id="contracts"
            style="border-radius: 0 !important;"
            @click="() => switchGrid('contracts')"
        >
          Реестр договоров (Юр.Лица)
        </button>
        <button
            class="content-h"
            id="agents"
            style="border-radius: 0 !important;"
            @click="() => switchGrid('agents')"
        >
          Реестр контрагентов
        </button>
        <button class="btn-plus" @click="openAddFolder">
          <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
        ...
        <button class="btn-i" @click="openSettings">
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
      ...
      <div class="contract-grid-container">
        <div class="side-navigation-menu-c" :style="!openState && 'margin-left: 30px'">
          <div class="compact-mode-switcher" @click="() => openState = !openState">
            <svg
                ...
                xmlns="http://www.w3.org/2000/svg"
            >
              <path v-if="openState" d="M15 6L9 12L15 18" stroke="white" stroke-width="2"/>
              <path v-if="!openState" d="M9 6L15 12L9 18" stroke="white" stroke-width="2"/>
            </svg>
          </div>
          <transition name="slide">
            <SearchDrawer v-if="openState" :show-abn="true" :show-contr="true"/>
          </transition>
        </div>
          <ContractsGrid
              v-if="activeGrid === 'contracts'"
              :data-source="dataContracts"
              :on-row-prepared="onRowPrepared"
              :on-cell-prepared="onCellPrepared"
              :on-open-click="onOpenClick"
          />
          <ContragentsGrid
              v-else-if="activeGrid === 'agents'"
              :data-source="dataAgents"
              :on-row-prepared="onRowPrepared"
              :on-cell-prepared="onCellPrepared"
              :on-open-click="onOpenClick"
          />
          ...
          <!--                    <option value="ca">Адрес</option>-->
          <!--                    <option value="usa">Дата</option>-->
          <!--                  </select>-->
          <!--                </div>-->
          <!--                <div class="filter-item-value">-->
          <!--                  <input class="filter-item-value-input" @input="oninput "/>-->
          <!--                </div>-->
...
import SearchDrawer from "@/views/search-drawer.vue";
// import {DxCheckBox} from "devextreme-vue";
import ContractsGrid from "@/components/contracts-grid.vue";
import ContragentsGrid from "@/components/contragents-grid.vue";
...
function getMainMenuHeight() {
  ...
  if (mainMenu && currentLayout) {
    ...
    if (mainMenu.clientWidth > 1172) {
      currentLayout.style.height = `calc(100% - ${mainMenu.clientHeight}px)`;
    } else {
      currentLayout.style.height = `calc(100% - ${
        mainMenu.clientHeight + 80
      }px)`;
    }
...
export default {
  ...
  setup() {
    ...
    const contragentList = contragent;
    ...
    function onOpenClick(event) {
      const kod_dog = event.data.KOD_DOG;
      let pops = store.getters["drawer/getPop"];
      const find_pop = pops.filter((item) => item.kod_dog === kod_dog);
      if (find_pop.length !== 0) {
        find_pop[0].pop.focus();
      } else {
        const contragent = contragentList.find(el => el.KOD_ABONENT === event.data.KOD_ABONENT);
        store.dispatch("card/setItemToContextHistory", contragent)
        const pop = window.open("/#/contract-card-view/" + kod_dog);
        store.dispatch("drawer/setPop", { kod_dog, pop: pop });
        console.log(store)
      }
    }
    ...
    function onCellPrepared(e) {
      if (e.rowType === "header") {
        ...
        e.cellElement.style.paddingBottom = "10px";
        e.cellElement.style.textAlign = "center";
      } else {
        e.cellElement.style.paddingTop = "1px";
  ...
  data() {
    return {
      ...
      openState: false,
      activeGrid: 'contracts',
      dataContracts: contractList,
      dataAgents: contragent,
      toolbarContent: [{
        widget: 'dxButton',
        location: 'before',
        options: {
  ...
  methods: {
    ...
    // },
    oninput(event) {
      //текст, который будет в input
      //event.target.value
      let newData = [];
      const contractsDataGrid = this.$refs["contractsDataGrid"].instance;
      if (event.target.value !== "") {
        newData = this.dataSource.filter((item) => {
          if (item.NDOG.toString().includes(event.target.value)) {
            return true;
          }
          if (item.DTE_NAME.toString().includes(event.target.value)) {
            return true;
          }
          if (item.VDOG_NAME.toString().includes(event.target.value)) {
            return true;
          }
          if (item.DATE_START.toString().includes(event.target.value)) {
            return true;
          }
          if (item.DATE_CONCLUSION.toString().includes(event.target.value)) {
            return true;
          }
          if (item.DATE_FIN.toString().includes(event.target.value)) {
            return true;
          }
          if (item.INN.toString().includes(event.target.value)) {
            return true;
          }
          if (item.KPP.toString().includes(event.target.value)) {
            return true;
          }
          if (item.NUMP.toString().includes(event.target.value)) {
            return true;
          }
          if (item.NAME.toString().includes(event.target.value)) {
            return true;
          }
          if (item.PL_NAME.toString().includes(event.target.value)) {
            return true;
          }
          if (item.OKVED.toString().includes(event.target.value)) {
            return true;
          }
          return false;
        });
      ...
      contractsDataGrid.option("dataSource", newData);
      console.log(event);
    },
    onClick(e) {
    ...
    openSettings() {
      window.open("/#/search-settings/");
    },
    openAddFolder() {
      window.open('/#/add-folder/')
    },
    switchGrid(name) {
      const btnContr = document.querySelector('button#contracts')
      const btnAgent = document.querySelector('button#agents')

      if (name !== this.activeGrid) {
        btnContr.classList.toggle('active-btn')
        btnAgent.classList.toggle('active-btn')
...
.content {
  ...
  #contracts-view {
    ...
    .contracts-content {
      ...
      .dx-drawer-wrapper, .dx-drawer-content, .dx-datagrid-rowsview {
        overflow-y: auto;
      }
        ...
        .filter-panel {
          ...
          .filter-container {
            ...
            @media all and (max-width: 1876px) {
              .filter-item {
                /* На экранах среднего размера мы центрируем элементы, равномерно распределяя пустое пространство вокруг элементов */
                width: 410px; /* Или что угодно */
              }
            }
            @media all and (max-width: 1655px) {
              .filter-item {
                /* На экранах среднего размера мы центрируем элементы, равномерно распределяя пустое пространство вокруг элементов */
                width: 350px; /* Или что угодно */
              }
            }
            @media all and (max-width: 1440px) {
              .filter-item {
                /* На экранах среднего размера мы центрируем элементы, равномерно распределяя пустое пространство вокруг элементов */
                width: 310px; /* Или что угодно */
              }
            }
            @media all and (max-width: 800px) {
              .filter-item {
                /* На экранах среднего размера мы центрируем элементы, равномерно распределяя пустое пространство вокруг элементов */
                width: 250px; /* Или что угодно */
              }
            }
          }
